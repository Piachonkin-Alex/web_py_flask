...
class TestArticleMethods(unittest.TestCase):
    ...
    def test_read(self):
        lib.read_articles(self.articles)
    ...
    def test_add(self):
        lib.read_articles(self.articles)
        lib.add_article(self.articles, "new_txt", 1, "second article", "alex")
        self.assertEqual(2, len(self.articles))
        self.assertEqual('new_txt', self.articles[1].text)
        copy_articles = []
        lib.read_articles(copy_articles)
        self.assertEqual(2, len(self.articles))

    def test_change(self):
        lib.add_article(self.articles, "new_txt", 0, "article", "alex")
        self.assertEqual("article", self.articles[0].title)
        lib.change_article(self.articles, "change_txt", 0, "new title", "Sasha")
